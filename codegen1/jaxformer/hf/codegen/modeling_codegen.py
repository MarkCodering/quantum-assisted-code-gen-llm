...
from qiskit_machine_learning.circuit.library import ZZFeatureMap, ZFeatureMap, RealAmplitudes
...
def create_qnn(num_qubits):
    feature_map = ZFeatureMap(num_qubits, reps=2)
...
qnn = create_qnn(num_qubits=num_qubits)

class CodeGenQuantumNN(nn.Module):
    def __init__(self, num_qubits, config):  # in MLP: intermediate_size= 4 * embed_dim
        super().__init__()
        embed_dim = config.n_embd

        self.fc_in = nn.Linear(embed_dim, num_qubits)
        self.qnn = TorchConnector(qnn)

    def forward(self, hidden_states):
        hidden_states = self.fc_in(hidden_states)
        hidden_states = self.qnn(hidden_states)
